...
const config = {
  d1_databases: [
    { binding: "DB", database_name: "aspt-nine-centers-cobalt-site" },
  ],
  env: {
    preview: {
      d1_databases: [
        { binding: "DB", database_name: "aspt-nine-centers-cobalt-preview-site" },
      ],
    },
    production: {
      d1_databases: [
        { binding: "DB", database_name: "aspt-nine-centers-cobalt-production-site" },
      ],
    },
  },
};
...
test("selects the D1 database from the requested deployment environment", () => {
  assert.equal(
    ...
    "aspt-nine-centers-cobalt-preview-site",
  );
  assert.equal(
    databaseNameForEnvironment(config, "production"),
    "aspt-nine-centers-cobalt-production-site",
  );
});
